'use client';

import { createContext, useContext, useState, useEffect } from 'react';
import { syncDocumentLanguage } from './documentLanguage';

const LANG_KEY = 'dreamvalley_lang';

const translations = {
  en: {
    appName: 'Dream Valley',
    // Onboarding
    onboardingTitle: 'Welcome to a world of magical stories',
    onboardingSubtitle: 'Bedtime stories, poems & songs crafted with love',
    onboardingChooseLang: 'Choose your language',
    onboardingContinue: 'Continue',
    // Auth
    loginTitle: 'What should we call you?',
    loginSubtitle: 'Pick a fun username to start your adventure',
    loginPlaceholder: 'Your dreamer name...',
    loginButton: 'Start Dreaming',
    loginLoading: 'Entering...',
    loginError: 'Oops! That didn\'t work. Try a different name.',
    // Navigation
    navHome: 'Home',
    navBeforeBed: 'Before Bed',
    navMyStories: 'My Stories',
    navProfile: 'Profile',
    // Home
    homeGreeting: 'Good evening',
    homeAllThemes: 'All',
    homeTrending: 'Popular Stories',
    homeStories: 'Short Stories',
    homePoems: 'Poems',
    homeSongs: 'Lullabies',
    // Themes
    themeFantasy: 'Fantasy',
    themeAdventure: 'Adventure',
    themeAnimals: 'Animals',
    themeSpace: 'Space',
    themeOcean: 'Ocean',
    themeForest: 'Forest',
    themeMagic: 'Magic',
    themeFriendship: 'Friendship',
    themeDreamy: 'Dreamy',
    themeFairyTale: 'Fairy Tales',
    // Before Bed
    beforeBedTitle: 'Before Bed',
    beforeBedEmpty: 'No before-bed content yet — check back soon!',
    // My Stories
    myContentTitle: 'My Content',
    myContentSubtitle: 'Your saved stories and creative tools',
    myCredits: 'Credits',
    myCharacters: 'Characters',
    myVoices: 'Voices',
    myCreateContent: 'Create Content',
    myCreateCharacter: 'Create Character',
    myRecordVoice: 'Record Voice',
    myComingSoon: 'Coming soon',
    myAvailableNow: 'Available now',
    myLocked: 'Locked',
    myMoonExplorer: 'Moon Explorer',
    myDreamGuardian: 'Dream Guardian',
    myGentleStoryteller: 'Gentle Storyteller',
    myMoonlightVoice: 'Moonlight Voice',
    myComingSoonClose: 'Got it',
    myContentComingTitle: 'Content creation is coming soon',
    myCharacterComingTitle: 'Character creation is coming soon',
    myVoiceComingTitle: 'Voice recording is coming later',
    myComingBody: 'We are preparing this feature for a future release.',
    characterTitle: 'Create Character',
    characterEditTitle: 'Edit Character',
    characterEditReview: 'Review changes',
    characterSteps: 'Character creation steps',
    characterIdentity: 'Identity',
    characterPersonality: 'Personality',
    characterReview: 'Review',
    characterName: 'Name',
    characterType: 'Type',
    characterGender: 'Gender',
    characterTraits: 'Traits',
    characterDetails: 'Details',
    characterChoose: 'Choose one',
    characterSurpriseName: 'Surprise name',
    characterSurpriseType: 'Surprise type',
    characterSurpriseGender: 'Surprise gender',
    characterContinue: 'Continue',
    characterBack: 'Back',
    characterSlot: 'Slot',
    characterFree: 'Free',
    characterCredits: 'credits',
    characterCreate: 'Create Character',
    characterConfirm: 'Confirm',
    characterCancel: 'Cancel',
    characterPaidTitle: 'Create for 2 credits?',
    characterPaidBody: 'This uses {cost} credits and leaves {balance} credits.',
    characterGenerating: 'Creating your character…',
    characterFailed: 'Could not create your character',
    characterConnectionFailed: 'Connection interrupted. Your character is still being created.',
    characterRetry: 'Retry',
    characterDone: 'Done',
    characterEdit: 'Edit',
    characterDelete: 'Delete',
    characterQuoteFailed: 'Could not refresh your quote',
    characterSubmitFailed: 'Could not start your character',
    characterErrorStaleQuote: 'Your balance or slot changed. Review the refreshed quote.',
    characterErrorInsufficientCredits: 'You do not have enough credits for this character.',
    characterErrorNoSlots: 'There are no character slots available right now.',
    characterDeleteConfirm: 'Delete this character?',
    characterDeleteFailed: 'Could not delete this character',
    characterValidationNameRequired: 'Enter a valid name or choose Surprise me',
    characterValidationNameLength: 'Keep names under 40 characters',
    characterValidationType: 'Choose a valid type or Surprise me',
    characterValidationGender: 'Choose a valid gender or Surprise me',
    characterValidationTraits: 'Choose valid traits',
    characterValidationTraitsLength: 'Choose up to 5 traits',
    characterValidationDetails: 'Enter details as text',
    characterValidationDetailsLength: 'Keep details under 300 characters',
    characterTypeHumanChild: 'Human child',
    characterTypeCat: 'Cat',
    characterTypeDog: 'Dog',
    characterTypeFox: 'Fox',
    characterTypeRabbit: 'Rabbit',
    characterTypeBear: 'Bear',
    characterTypeBird: 'Bird',
    characterTypeDragon: 'Dragon',
    characterTypeUnicorn: 'Unicorn',
    characterTypeRobot: 'Robot',
    characterTypeMermaid: 'Mermaid',
    characterTypeFairy: 'Fairy',
    characterTypeNatureSpirit: 'Nature spirit',
    characterGenderGirl: 'Girl',
    characterGenderBoy: 'Boy',
    characterGenderNonBinary: 'Non-binary',
    characterGenderNotSpecified: 'Not specified',
    characterTraitBrave: 'Brave',
    characterTraitCurious: 'Curious',
    characterTraitKind: 'Kind',
    characterTraitPlayful: 'Playful',
    characterTraitGentle: 'Gentle',
    characterTraitWise: 'Wise',
    characterTraitFunny: 'Funny',
    characterTraitShy: 'Shy',
    characterTraitCreative: 'Creative',
    characterTraitLoyal: 'Loyal',
    characterTraitAdventurous: 'Adventurous',
    characterTraitCalm: 'Calm',
    characterTraitDreamy: 'Dreamy',
    characterTraitClever: 'Clever',
    myStoriesTitle: 'My Stories',
    myStoriesSubtitle: 'Your favorite stories, all in one place',
    myFavorites: 'Favorites',
    mySaved: 'Saved',
    myEmptyFavorites: 'No favorites yet',
    myEmptyFavoritesText: 'Tap the heart on any story to add it here!',
    myEmptySaved: 'No saved stories yet',
    myEmptySavedText: 'Save stories to read them again anytime!',
    myExplore: 'Explore Stories',
    // Player
    playerBack: 'Back',
    playerLike: 'Like',
    playerSave: 'Save',
    playerSaved: 'Saved',
    playerSavedToProfile: 'Saved to your profile',
    playerRemovedFromSaved: 'Removed from saved',
    heartCapTitle: 'Your 5 free saves are full',
    heartCapBody: 'Upgrade to Premium for 30 saved favorites and offline listening.',
    heartCapUpgrade: 'Upgrade to Premium',
    heartCapDismiss: 'Close',
    premiumCapTitle: 'Your Premium library is full',
    premiumCapBody: 'Your Premium library is full. Remove one saved favorite to add another.',
    heartSignInToSave: 'Sign in to save',
    playerShare: 'Share',
    playerShareCopied: 'Link Copied!',
    playerAbout: 'About',
    playerReport: 'Report',
    playerReportTitle: 'Report an Issue',
    playerReportStory: 'Story',
    playerReportVoice: 'Voice',
    playerReportIssueType: 'What went wrong?',
    playerReportAudioQuality: 'Audio quality issue',
    playerReportStoryContent: 'Story content issue',
    playerReportBackgroundMusic: 'Background music issue',
    playerReportVoiceMismatch: 'Voice doesn\'t match story',
    playerReportOther: 'Other issue',
    playerReportDescription: 'Tell us more (optional)',
    playerReportSubmit: 'Submit Report',
    playerReportCancel: 'Cancel',
    playerReportSent: 'Report sent to support@dreamvalley.app',
    playerReportError: 'Could not send report. Please try again.',
    playerLoading: 'Loading your story...',
    playerError: 'Could not load this story',
    playerNotFound: 'Story not found',
    playerNoContent: 'No content available',
    playerExplore: 'Explore Stories',
    playerMinRead: 'min read',
    playerStory: 'Short Story',
    playerPoems: 'Poems',
    playerSongs: 'Lullabies',
    playerQA: 'Questions & Answers',
    // Content types
    typeStory: 'Short Story',
    typePoem: 'Poem',
    typeSong: 'Lullaby',
    // Lengths
    lengthShort: 'Short',
    lengthMedium: 'Medium',
    lengthLong: 'Long',
    lengthShortDetail: '2-3 min',
    lengthMediumDetail: '5-7 min',
    lengthLongDetail: '10-15 min',
    // Profile
    profileTitle: 'Profile',
    profileLogout: 'Logout',
    profileLanguage: 'Language',
    // Voice Onboarding
    voicePreviewTitle: 'Choose Your Narrator',
    voicePreviewSubtitle: 'Tap to preview, select your favorite',
    voiceSettings: 'Voice Settings',
    voiceSettingsDesc: 'Choose your preferred narrator voice',
    getStarted: 'Get Started',
    // Misc
    loading: 'Loading...',
    minRead: 'min read',
    noStories: 'No stories yet',
  },
  hi: {
    appName: 'Sapno ki Duniya',
    // Onboarding
    onboardingTitle: 'Jaadui kahaniyon ki duniya mein aapka swagat hai',
    onboardingSubtitle: 'Sone se pehle ki kahaniyan, kavitayen aur gaane, pyaar se banaaye gaye',
    onboardingChooseLang: 'Apni bhasha chuniye',
    onboardingContinue: 'Aage badhein',
    // Auth
    loginTitle: 'Hum aapko kya bulaayein?',
    loginSubtitle: 'Ek mazedaar naam chuniye aur shuru karein',
    loginPlaceholder: 'Aapka sapno wala naam...',
    loginButton: 'Sapne dekhna shuru karein',
    loginLoading: 'Aa rahe hain...',
    loginError: 'Oho! Kuch gadbad ho gayi. Doosra naam try karein.',
    // Navigation
    navHome: 'Home',
    navBeforeBed: 'Sone Se Pehle',
    navMyStories: 'Meri Kahaniyan',
    navProfile: 'Profile',
    // Home
    homeGreeting: 'Shubh sandhya',
    homeAllThemes: 'Sabhi',
    homeTrending: 'Lokpriya Kahaniyan',
    homeStories: 'Kahaniyan',
    homePoems: 'Kavitayen',
    homeSongs: 'Loriyaan',
    // Themes
    themeFantasy: 'Kalpana',
    themeAdventure: 'Sahas',
    themeAnimals: 'Janwar',
    themeSpace: 'Antariksh',
    themeOcean: 'Samundar',
    themeForest: 'Jungle',
    themeMagic: 'Jaadu',
    themeFriendship: 'Dosti',
    themeDreamy: 'Sapne',
    themeFairyTale: 'Pari Kathayein',
    // Before Bed
    beforeBedTitle: 'Sone Se Pehle',
    beforeBedEmpty: 'Abhi koi mazedaar short nahi — jaldi aayega!',
    // My Stories
    myContentTitle: 'Mera Content',
    myContentSubtitle: 'Aapki save ki hui kahaniyan aur creative tools',
    myCredits: 'Credits',
    myCharacters: 'Kirdaar',
    myVoices: 'Aawaazein',
    myCreateContent: 'Content Banayein',
    myCreateCharacter: 'Kirdaar Banayein',
    myRecordVoice: 'Aawaaz Record Karein',
    myComingSoon: 'Jaldi aa raha hai',
    myAvailableNow: 'Abhi available hai',
    myLocked: 'Band hai',
    myMoonExplorer: 'Chaand ka Khoji',
    myDreamGuardian: 'Sapnon ka Rakhwala',
    myGentleStoryteller: 'Pyaara Kahanikaar',
    myMoonlightVoice: 'Chaandni ki Aawaaz',
    myComingSoonClose: 'Theek hai',
    myContentComingTitle: 'Content banana jaldi aa raha hai',
    myCharacterComingTitle: 'Kirdaar banana jaldi aa raha hai',
    myVoiceComingTitle: 'Aawaaz record karna baad mein aayega',
    myComingBody: 'Hum is feature ko aane wali release ke liye taiyar kar rahe hain.',
    characterTitle: 'Kirdaar Banayein',
    characterEditTitle: 'Kirdaar edit karein',
    characterEditReview: 'Badlaav review karein',
    characterSteps: 'Kirdaar banane ke steps',
    characterIdentity: 'Pehchaan',
    characterPersonality: 'Mizaaj',
    characterReview: 'Review',
    characterName: 'Naam',
    characterType: 'Type',
    characterGender: 'Gender',
    characterTraits: 'Khaas baatein',
    characterDetails: 'Details',
    characterChoose: 'Chuniye',
    characterSurpriseName: 'Naam surprise rakhein',
    characterSurpriseType: 'Type surprise rakhein',
    characterSurpriseGender: 'Gender surprise rakhein',
    characterContinue: 'Aage badhein',
    characterBack: 'Wapas',
    characterSlot: 'Slot',
    characterFree: 'Free',
    characterCredits: 'credits',
    characterCreate: 'Kirdaar Banayein',
    characterConfirm: 'Confirm karein',
    characterCancel: 'Raddh Karein',
    characterPaidTitle: '2 credits mein banayein?',
    characterPaidBody: 'Ismein {cost} credits lagenge aur {balance} credits bachenge.',
    characterGenerating: 'Aapka kirdaar ban raha hai…',
    characterFailed: 'Aapka kirdaar nahi ban paaya',
    characterConnectionFailed: 'Connection ruk gaya. Aapka kirdaar ab bhi ban raha hai.',
    characterRetry: 'Dobara try karein',
    characterDone: 'Ho gaya',
    characterEdit: 'Edit',
    characterDelete: 'Delete',
    characterQuoteFailed: 'Aapka quote refresh nahi ho paaya',
    characterSubmitFailed: 'Kirdaar shuru nahi ho paaya',
    characterErrorStaleQuote: 'Aapka balance ya slot badal gaya. Naya quote review karein.',
    characterErrorInsufficientCredits: 'Is kirdaar ke liye aapke paas kaafi credits nahi hain.',
    characterErrorNoSlots: 'Abhi koi character slot available nahi hai.',
    characterDeleteConfirm: 'Kya is kirdaar ko delete karna hai?',
    characterDeleteFailed: 'Kirdaar delete nahi ho paaya',
    characterValidationNameRequired: 'Valid naam likhein ya Surprise me chunein',
    characterValidationNameLength: 'Naam 40 characters se chhota rakhein',
    characterValidationType: 'Valid type chunein ya Surprise me chunein',
    characterValidationGender: 'Valid gender chunein ya Surprise me chunein',
    characterValidationTraits: 'Valid traits chunein',
    characterValidationTraitsLength: 'Zyada se zyada 5 traits chunein',
    characterValidationDetails: 'Details text mein likhein',
    characterValidationDetailsLength: 'Details 300 characters se chhoti rakhein',
    characterTypeHumanChild: 'Insaan bachcha',
    characterTypeCat: 'Billi',
    characterTypeDog: 'Kutta',
    characterTypeFox: 'Lomdi',
    characterTypeRabbit: 'Khargosh',
    characterTypeBear: 'Bhaloo',
    characterTypeBird: 'Panchhi',
    characterTypeDragon: 'Dragon',
    characterTypeUnicorn: 'Unicorn',
    characterTypeRobot: 'Robot',
    characterTypeMermaid: 'Jalpari',
    characterTypeFairy: 'Pari',
    characterTypeNatureSpirit: 'Prakriti ki rooh',
    characterGenderGirl: 'Ladki',
    characterGenderBoy: 'Ladka',
    characterGenderNonBinary: 'Non-binary',
    characterGenderNotSpecified: 'Batana nahi hai',
    characterTraitBrave: 'Bahadur',
    characterTraitCurious: 'Jigyasu',
    characterTraitKind: 'Dayalu',
    characterTraitPlayful: 'Masti bhara',
    characterTraitGentle: 'Komal',
    characterTraitWise: 'Samajhdaar',
    characterTraitFunny: 'Mazakiya',
    characterTraitShy: 'Sharmila',
    characterTraitCreative: 'Rachnatmak',
    characterTraitLoyal: 'Wafadar',
    characterTraitAdventurous: 'Sahasi',
    characterTraitCalm: 'Shaant',
    characterTraitDreamy: 'Sapnon jaisa',
    characterTraitClever: 'Chalaak',
    myStoriesTitle: 'Meri Kahaniyan',
    myStoriesSubtitle: 'Aapki pasandida kahaniyan, ek jagah pe',
    myFavorites: 'Pasandida',
    mySaved: 'Save ki hui',
    myEmptyFavorites: 'Abhi koi pasandida nahi',
    myEmptyFavoritesText: 'Kisi bhi kahani pe dil daba ke yahan jodein!',
    myEmptySaved: 'Abhi kuch save nahi hai',
    myEmptySavedText: 'Kahaniyan save karein taaki phir se padh sakein!',
    myExplore: 'Kahaniyan dekhein',
    // Player
    playerBack: 'Wapas',
    playerLike: 'Pasand',
    playerSave: 'Save',
    playerSaved: 'Save hai',
    playerSavedToProfile: 'Aapki profile mein save ho gaya',
    playerRemovedFromSaved: 'Save se hata diya',
    heartCapTitle: 'Aapke 5 free saves bhar gaye hain',
    heartCapBody: '30 pasandida kahaniyan save karne aur offline sunne ke liye Premium lein.',
    heartCapUpgrade: 'Premium lein',
    heartCapDismiss: 'Band karein',
    premiumCapTitle: 'Aapki Premium library bhar gayi hai',
    premiumCapBody: 'Aapki Premium library bhar gayi hai. Nayi pasandida kahani jodne ke liye ek saved kahani hatayein.',
    heartSignInToSave: 'Save karne ke liye sign in karein',
    playerShare: 'Share',
    playerShareCopied: 'Link Copy hua!',
    playerAbout: 'Parichay',
    playerReport: 'Report',
    playerReportTitle: 'Samasya Batayein',
    playerReportStory: 'Kahani',
    playerReportVoice: 'Awaaz',
    playerReportIssueType: 'Kya gadbad hai?',
    playerReportAudioQuality: 'Awaaz ki quality kharab hai',
    playerReportStoryContent: 'Kahani mein koi samasya hai',
    playerReportBackgroundMusic: 'Background sangeet mein samasya',
    playerReportVoiceMismatch: 'Awaaz kahani se match nahi karti',
    playerReportOther: 'Koi aur samasya',
    playerReportDescription: 'Aur batayein (optional)',
    playerReportSubmit: 'Report Bhejein',
    playerReportCancel: 'Raddh Karein',
    playerReportSent: 'Report support@dreamvalley.app ko bhej di gayi',
    playerReportError: 'Report nahi bhej paayi. Dobara koshish karein.',
    playerLoading: 'Aapki kahani load ho rahi hai...',
    playerError: 'Ye kahani load nahi ho paayi',
    playerNotFound: 'Kahani nahi mili',
    playerNoContent: 'Koi content nahi hai',
    playerExplore: 'Kahaniyan dekhein',
    playerMinRead: 'min padhne mein',
    playerStory: 'Kahani',
    playerPoems: 'Kavitayen',
    playerSongs: 'Loriyaan',
    playerQA: 'Sawaal Jawaab',
    // Content types
    typeStory: 'Kahani',
    typePoem: 'Kavita',
    typeSong: 'Lori',
    // Lengths
    lengthShort: 'Chhoti',
    lengthMedium: 'Beech ki',
    lengthLong: 'Lambi',
    lengthShortDetail: '2-3 min',
    lengthMediumDetail: '5-7 min',
    lengthLongDetail: '10-15 min',
    // Profile
    profileTitle: 'Profile',
    profileLogout: 'Bahar jaayein',
    profileLanguage: 'Hindi Level',
    // Voice Onboarding
    voicePreviewTitle: 'Apna Kathakaar Chunein',
    voicePreviewSubtitle: 'Sunne ke liye tap karein, apni pasand chunein',
    voiceSettings: 'Awaaz Settings',
    voiceSettingsDesc: 'Apni pasandida awaaz chunein',
    getStarted: 'Shuru Karein',
    // Misc
    loading: 'Load ho raha hai...',
    minRead: 'min padhne mein',
    noStories: 'Abhi koi kahani nahi',
  },
};

const I18nContext = createContext(null);

export function I18nProvider({ children }) {
  const [lang, setLangState] = useState('en');
  const [ready, setReady] = useState(false);

  useEffect(() => {
    const saved = localStorage.getItem(LANG_KEY);
    setLangState(saved === 'hi' ? 'hi' : 'en');
    setReady(true);
  }, []);

  useEffect(() => {
    syncDocumentLanguage(lang);
  }, [lang]);

  const setLang = (newLang) => {
    const safeLang = newLang === 'hi' ? 'hi' : 'en';
    setLangState(safeLang);
    localStorage.setItem(LANG_KEY, safeLang);
  };

  const t = (key) => {
    return translations[lang]?.[key] || translations.en[key] || key;
  };

  const hasLanguage = () => {
    if (typeof window === 'undefined') return false;
    return !!localStorage.getItem(LANG_KEY);
  };

  return (
    <I18nContext.Provider value={{ lang, setLang, t, ready, hasLanguage }}>
      {children}
    </I18nContext.Provider>
  );
}

export function useI18n() {
  const ctx = useContext(I18nContext);
  if (!ctx) throw new Error('useI18n must be used within I18nProvider');
  return ctx;
}

export function getLang() {
  if (typeof window === 'undefined') return 'en';
  return localStorage.getItem(LANG_KEY) || 'en';
}

export function hasCompletedOnboarding() {
  if (typeof window === 'undefined') return false;
  // Logged-in users: defer to the cached backend user. Treat
  // onboarding_complete !== false as complete (true or undefined,
  // the latter covering legacy pre-backfill records). Without this
  // branch, the 53 existing users — who never wrote the anon
  // localStorage keys — would be force-routed to /onboarding.
  let cached = null;
  try { cached = JSON.parse(localStorage.getItem('dreamweaver_user') || 'null'); } catch {}
  if (cached) {
    return cached.onboarding_complete !== false;
  }
  // Anon: require the full localStorage profile (lang + username + age).
  return !!localStorage.getItem(LANG_KEY)
    && !!localStorage.getItem('dreamvalley_anon_username')
    && !!localStorage.getItem('dreamvalley_child_age');
}
