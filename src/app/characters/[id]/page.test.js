...
test('idle delete dialog focuses cancel, traps Tab, and returns focus after Escape', async () => { const host=document.createElement('div'); const root=createRoot(host); document.body.appendChild(host); await act(async()=>{root.render(<CharacterDetailPage params={{id:'c1'}}/>); await Promise.resolve(); await Promise.resolve();}); const trigger=Array.from(host.querySelectorAll('button')).find((button)=>button.textContent==='Delete Character'); trigger.focus(); await act(async()=>trigger.click()); const dialog=host.querySelector('[role="dialog"]'); expect(document.activeElement.textContent).toBe('Cancel'); await act(async()=>dialog.dispatchEvent(new KeyboardEvent('keydown',{key:'Escape',bubbles:true}))); expect(host.querySelector('[role="dialog"]')).toBeNull(); expect(document.activeElement).toBe(trigger); await act(async()=>root.unmount()); host.remove(); });
