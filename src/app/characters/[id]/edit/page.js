...
export default function EditCharacterPage({ params }) {
  ...
  const profile = character.profile || character;
  return <main className={styles.page}><div className={styles.card}><header className={styles.hero} aria-label={t('characterHeroLabel')}><span className={styles.heroMoon} aria-hidden="true" /><div><p className={styles.heroEyebrow}>Dream Valley</p><h2>{t('characterHeroTitle')}</h2><p>{t('characterHeroSubtitle')}</p></div></header>{!showWizardResult && <>{character.portrait_url && <img className={styles.editPortrait} src={character.portrait_url} alt="" />}<h1>{profile.name}</h1><p>{profile.profile_summary}</p></>}<CharacterWizard key={wizardVersion} uid={user.uid || user.id} mode="edit" targetCharacterId={params.id} initialInputs={inputsFor(character)} onResult={(saved) => { setCharacter(saved); setShowWizardResult(true); }} onDone={() => router.replace(`/characters/${params.id}`)} onEdit={() => { setShowWizardResult(false); setWizardVersion((version) => version + 1); }} onDelete={() => router.replace('/my-stories')} /></div></main>;
}
